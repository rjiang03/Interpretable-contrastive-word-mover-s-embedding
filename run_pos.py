...
def train(model, args, look_up, grade, length, weight, num_class, seed):
    epoch_num = 50
    acc = np.zeros((epoch_num+1, 4))
    for j in range(1): 
        print(j)
        seed = 4
        index_train = np.load('index_tr.npy')[seed].astype(int)-1
        index_test = np.load('index_te.npy')[seed].astype(int)-1
        ...
        #criterion = Loss_reference_neg_3()
        '''
        ...
        for epoch in range(epoch_num):
            ...
            acc[epoch][0] = loss.cpu().detach().numpy()
            ...
            if epoch > 10:
                ...
                if val_acc >= val_best:
                    ...
                    print('test_acc', test_qwk)
                    acc[epoch][3] = test_qwk
            ...
            if epoch % 20 == 0:
                ...
                print('train', train_qwk)
                acc[epoch][1] = train_qwk

            torch.save(model,'model'+str(seed)+'_new.pkl')
            np.save('acc'+str(seed)+'_.npy', acc)
...
if __name__ == '__main__':
    ...
    train(model, args, look_up, grade, length, weight, num_class, seed)
